...
def get_partic_ratio(explained_variance):
    num = (np.sum(explained_variance))**2
    den = np.sum(explained_variance**2)
    return num/den


def get_reps_by_class(class_dict, rep_transform=None, normalize=True, **kwargs):
    reps_by_class = []

    for datapoints in class_dict.values():

        if rep_transform:
            one_class_reps = rep_transform(datapoints, **kwargs)
        else:
            one_class_reps = datapoints
        
        if normalize == True:
            one_class_reps /= np.linalg.norm(one_class_reps, axis=0)[None,:]
    
        reps_by_class.append(np.array(one_class_reps))

    return(reps_by_class)
